...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if context.args:
            ...
            if ref_id != user_id:
                ...
                await context.bot.send_message(
                    chat_id=ref_id,
                    text=f"🎉 Bạn vừa nhận {REF_REWARD}đ vì mời được 1 người!"
                )
    ...
    if all_joined:
        await update.message.reply_text(
            "✅ Bạn đã tham gia đủ kênh. Chọn chức năng bên dưới:",
            reply_markup=menu_keyboard()
        )
    else:
        channel_list = "\n".join([f"➡️ {ch}" for ch in REQUIRED_CHANNELS])
        await update.message.reply_text(
            f"❗ Bạn cần tham gia các kênh sau để sử dụng bot:\n{channel_list}\n\nSau khi tham gia, vui lòng bấm /start lại."
        )
